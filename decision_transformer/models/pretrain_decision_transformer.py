...
class PretrainDecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.predict_inverse = torch.nn.Linear(hidden_size, self.act_dim)
        self.predict_rand_inverse = torch.nn.Linear(hidden_size, self.act_dim)
        ...
        self.total_pretrain_steps = total_pretrain_steps
    ...
    def forward(
        ...
        ln_inputs = self.embed_ln(stacked_inputs)
        ...
        reward_preds = self.predict_return(x[:, 2])
        # predict next state given state and action
        state_preds = self.predict_state(x[:, 2])

        inverse_preds, randinv_preds = None, None

        if unsup_losses:
            ## inverse prediction
            inverse_transformer_outputs = self.transformer(
                inputs_embeds=ln_inputs,
                attention_mask=stacked_padding_mask,
                attn_type='inverse'
            )
            x_inv = inverse_transformer_outputs["last_hidden_state"]
            x_inv = x_inv.reshape(batch_size, seq_length, 3, self.hidden_size).permute(0, 2, 1, 3)
            inverse_preds = self.predict_inverse(x_inv[:, 1])      

            ## rand inverse prediction
            # mask all actions
            masked_inputs = stacked_inputs.clone()
            masked_inputs[:,2::3,:] = -1
            # mask some observations
            mask_obs_size = 5
            rand_mask_obs_idx = np.random.choice(list(range(1, seq_length-1)), mask_obs_size, replace=False)
            for j in range(mask_obs_size):
                masked_inputs[:, 1 + 3 * rand_mask_obs_idx[j], :] = -1 
            masked_inputs = self.embed_ln(masked_inputs) 
            randinv_transformer_outputs = self.transformer(
                inputs_embeds=masked_inputs,
                attention_mask=stacked_padding_mask,
                attn_type='full'
            )
            x_randinv = randinv_transformer_outputs["last_hidden_state"]
            x_randinv = x_randinv.reshape(batch_size, seq_length, 3, self.hidden_size).permute(0, 2, 1, 3)
            randinv_preds = self.predict_rand_inverse(x_randinv[:, 1]) 

        preds = {
            'state_preds': state_preds,
            'action_preds': action_preds,
            'reward_preds': reward_preds,
            'inverse_preds': inverse_preds,
            'randinv_preds': randinv_preds
        }
        return preds
    ...
    def get_predictions(
        ...
        state_preds, action_preds, return_preds = preds['state_preds'], preds['action_preds'], preds['reward_preds']
