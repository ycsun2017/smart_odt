...
class DiagGaussianActor(nn.Module):
    ...
    def __init__(self, hidden_dim, act_dim, log_std_bounds=[-5.0, 2.0]):
        ...
        self.mu = nn.Sequential(*[nn.Linear(hidden_dim*2, hidden_dim), nn.Tanh(), nn.Linear(hidden_dim, act_dim)])
        self.log_std = nn.Sequential(*[nn.Linear(hidden_dim*2, hidden_dim), nn.Tanh(), nn.Linear(hidden_dim, act_dim)])
...
class NewDecisionTransformer(TrajectoryModel):

    """
    New architecture of decision transformer that moves Returns to the policy head
    ...
    def __init__(
        ...
        **kwargs
        ...
        if stochastic_policy:
            # self.process_return = nn.Sequential(
            #     *(
    ...
    def forward(
        ...
        obs_rtg = (
            torch.stack(
                (x[:, 0], returns_embeddings), dim=1
            )
            .permute(0, 2, 1, 3)
            .reshape(batch_size, seq_length, 2*self.hidden_size)
        )

        action_preds = self.predict_action(obs_rtg)
